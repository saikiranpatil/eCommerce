...
export const getProducts = (keyword = "", currentPage = 1, price = [0, 30000], category = "") => async (dispatch) => {
    try {
        dispatch({ type: ALL_PRODUCT_REQUEST });

        let link = `/api/v1/products?page=${currentPage}${keyword === "" ? "" : `&keyword=${keyword}`}&price[gte]=${price[0]}&price[lte]=${price[1]}${category === "" ? "" : `&category=${category}`}`;
...
export const updateProduct = (id, productData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`/api/v1/user/product/${id}`, productData, { config });
